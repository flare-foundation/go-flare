...
Module: `github.com/ava-labs/avalanchego` | Go 1.25.8 (see `go.mod` toolchain) | See parent `../AGENTS.md` for the full merge guide.
...
- `Every()` returns immediately for non-positive frequency (disables that gossip loop; audit fix). Upstream substitutes a default period instead — preserve the Flare behavior on merge.
