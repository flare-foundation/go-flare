...
/**
 * Get OApp contract address by EID from LayerZero config or FlareOFTFeeUpgradeable deployment
 */
async function getOAppAddressByEid(
    ...
    // First try to get from FlareOFTFeeUpgradeable_Proxy deployment
    try {
        const flareOftDeployment = await hre.deployments.get('FlareOFTFeeUpgradeable_Proxy')
        if (flareOftDeployment && flareOftDeployment.address) {
            logger.info(`Using FlareOFTFeeUpgradeable_Proxy deployment address: ${flareOftDeployment.address}`)
            return flareOftDeployment.address
        }
    } catch (error) {
        logger.debug(`FlareOFTFeeUpgradeable_Proxy deployment not found: ${error}`)
    }

    // Fallback to LayerZero config approach
    try {
        ...
        logger.debug(`LayerZero config approach failed: ${configError}`)
        throw new Error(
            `Could not find OFT address for EID ${eid}. Tried:\n` +
                `1. FlareOFTFeeUpgradeable_Proxy deployment (not found)\n` +
                `2. LayerZero config at ${oappConfig} (failed: ${configError})\n` +
                `Please ensure FlareOFTFeeUpgradeable_Proxy is deployed or provide --oft-address parameter`
        )
    }
}
