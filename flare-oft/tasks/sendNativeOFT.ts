...
import { NativeArgs, sendNative } from './sendNative'
...
interface NativeMasterArgs {
    ...
    /** Arbitrary bytes message to deliver alongside the native OFT */
    composeMsg?: string
    /** EVM: 20-byte hex address of the Native OFT Adapter */
...
task('lz:oft:send:native', 'Sends native tokens cross‐chain from EVM chains using NativeOFTAdapterUpgradeable')
    .addParam('srcEid', 'Source endpoint ID', undefined, types.int)
    .addParam('dstEid', 'Destination endpoint ID', undefined, types.int)
    .addParam('amount', 'Amount of native tokens to send (human readable units, e.g. "1.5")', undefined, types.string)
    ...
    .addOptionalParam(
        ...
        'Override the source local deployment Native OFT Adapter address (20-byte hex for EVM)',
        undefined,
        types.string
    )
    .addFlag(
    ...
    .setAction(async (args: NativeMasterArgs, hre: HardhatRuntimeEnvironment) => {
        ...
        if (args.nativeAdapterAddress) {
            DebugLogger.printWarning(
                KnownWarnings.USING_OVERRIDE_OFT,
                `For network: ${endpointIdToNetwork(args.srcEid)}, Native OFT Adapter: ${args.nativeAdapterAddress}`
            )
        }
        ...
        if (chainType === ChainType.EVM) {
            ...
            result = await sendNative(args as NativeArgs, hre)
        } else {
            throw new Error(
                `The chain type ${chainType} is not supported in this Native OFT example. Only EVM chains are supported.`
            )
        }

        DebugLogger.printLayerZeroOutput(
            KnownOutputs.SENT_VIA_OFT,
            `Successfully sent ${args.amount} native tokens from ${endpointIdToNetwork(args.srcEid)} to ${endpointIdToNetwork(args.dstEid)}`
        )
